...
function navTabAjaxDone(json){
	...
	if (json[DWZ.keys.statusCode] == DWZ.statusCode.ok){
		...
		/***********************************此处为扩展DWZ框架***start***************************************/
		if("closeCurrentAndForward" == json.callbackType) {
			setTimeout(function(){
				navTab.closeCurrentTab();
				var $title = json.navTabTitle;
				var $fresh = true;
				var $external = false;
				navTab.openTab(json.navTabId, json.forwardUrl, {title:$title, fresh:$fresh, external:$external});
			}, 100);
		} else if ("openCurrentAndForward" == json.callbackType) {
			setTimeout(function(){
				var $title = json.navTabTitle;
				var $fresh = true;
				var $external = false;
...
/******************************************扩展dwz框架**start******************************************/
/**
 * 处理navTab上的提交按钮, 会新开一个navTab
 * @param {Object} form
 */
function navTabOpenTab(form, navTabTitle){
    var $form = $(form);
    navTab.openTab("test", "genCode/querySingleTable",{title:"预览",data: $form.serializeArray()});
    // navTab.openTab(navTabTitle, $form.attr('action'),{title: navTabTitle, data: $form.serializeArray()});
    return true;
}
/******************************************扩展dwz框架**ecd******************************************/
