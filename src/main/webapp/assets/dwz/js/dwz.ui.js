...
function initUI($p){
    ...
    $("a[target=navTabIds]", $p).each(function(){
        function _getIdsParam(selectedIds, targetType){
            var ids = "";
            var $box = targetType == "dialog" ? $.pdialog.getCurrent() : navTab.getCurrentPanel();
            $box.find("input:checked").filter("[name='"+selectedIds+"']").each(function(i){
                var val = $(this).val();
                var param = selectedIds + "=" + val;
                ids += i==0 ? param : "&" + param;
            });
            return ids;
        }
        $(this).click(function(event){
            ...
            var external = eval($this.attr("external") || "false");
            var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
            var ids = _getIdsParam(selectedIds, "map");
            url = url.indexOf("?") > -1 ? url + ids : url + "?" + ids;
            DWZ.debug(url);
            if (!url.isFinishedTm()) {
                alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
                return false;
            }
            navTab.openTab(ids, url,{title:title, fresh:fresh, external:external});
