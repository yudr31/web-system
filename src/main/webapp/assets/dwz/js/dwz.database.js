	...
	$.extend({
		bringBackSuggest: function(args){
			...
			$box.find(":input").each(function(){
                ...
                /**********************扩展-start************************/
				var lookupback = $input.attr("lookupback");
				for (var key in args) {
					var name = (_lookup.pk == key) ? _util.lookupPk(key) : _util.lookupField(key);

					if (name == lookupback || name == inputName) {
						$input.val(args[key]);
						break;
					}
				}
		...
		bringBack: function(args, callback){
			$.bringBackSuggest(args);
            callback && callback(args);	// 扩展dwz
		...
		multLookup: function(){
			return this.each(function(){
				...
				$this.click(function(event){
                    ...
                    var _callBack = $this.attr("multlookupcallback");
                    if(_callBack) {
                        _callBack = eval(_callBack);
                    }

					$.bringBack(args,_callBack);
				});
			});
		},
		suggest: function(){
